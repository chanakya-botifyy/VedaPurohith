...
import { FormControl, Grid, Input, InputLabel, MenuItem, Stack, TextField, Typography, styled } from '@mui/material';
...
const CustomeUpload = styled(TextField)({
    height:'40px',
    marginTop:'5px',
    '& .MuiInputBase-root':{
        borderRadius: 12,
    ...
    '& .MuiInputBase-input button': {
        borderRadius: 12,
        backgroundColor:'linear-gradient(#02C5DC, #0292D7)'
    }
})
export default function AddSeva(){
    ...
    async function addSeva(){
        ...
        try{
            const request = await instance.post('/addSeva',formData)
            console.log(request);
    ...
    return(
        <form onSubmit={(e) => { e.preventDefault(); addSeva()}}>
            <PageInfoBar 
                title = {'Sevas'} 
                button1={{name:'Export', icon:Canceled, onclick:onExport }}
                button2={{name:'Add Sevas', icon:Add, onclick:onAdd, type:'submit' }} 
            />
